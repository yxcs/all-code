class ImgLoader {
    ...
    constructor(pageContext, defaultCallback) {
        ...
        this.defaultCallback = defaultCallback || this._imageOnLoad.bind(this);
        this.callbacks = {}
        this.imgInfo = {}
        ...
        this.page._loadImages = this._loadImages.bind(this)
        this.page._imageOnLoad = this._imageOnLoad.bind(this)
    }
    ...
    _loadImages(list) {
        let imgList = list || this.page.data.imgList;
        imgList.forEach(item => {
            this.load(item.url)
        })
    }
    //加载完成后的回调
    _imageOnLoad(err, data) {
        console.log('图片加载完成')
        const imgList = this.page.data.imgList.map(item => {
            if (item.url == data.src) {
            item.loaded = true;
            }
            return item;
        })
        this.page.setData({ imgList });
    }
}
