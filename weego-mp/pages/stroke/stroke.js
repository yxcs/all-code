...
Page({
  ...
  onLoad: function (options) {
    this.setData({ id: '5b2cb6a84c4bb8128b016c6e' || options.oId });
    getOrderDetail('5b2cb6a84c4bb8128b016c6e' || options.oId).then(data => {
      ...
      datas = datas.data;
      if (datas.product_type === 'packages') {
        datas.hotel =
        datas.product_snapshot.hotels && datas.product_snapshot.hotels.length
            ? datas.product_snapshot.hotels[0].hotel
            : '';
            datas.reserveInfo = datas.package_conform_info[0] || {};
      }
      if (datas.product_type === 'hotels') {
        let customers = '';
        datas.customers.forEach((item, index) => {
          customers += `${item.first_name} ${item.last_name}${
            index === datas.customers.length - 1 ? '' : ', '
          }`;
        });
        datas.hotel = datas.product_snapshot;
        datas.reserveInfo = {
          check_in_date: datas.hotel_room_info.checkInTime,
          check_out_date: datas.hotel_room_info.checkOutTime,
          room_count: datas.hotel_room_info.roomCount,
          room_type: datas.hotel_room_info.detail.room_type,
          breakfast: datas.hotel_room_info.mealType,
          person_count: datas.customers.length,
