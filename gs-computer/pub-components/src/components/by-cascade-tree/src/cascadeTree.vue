      ...
      <div :class="['cascade--tree__operate', item._key === activeKey ? 'active' : '']">
        ...
        <template v-if="item._subLen">
          <span class="arrow" @click="showSecondSelection(item._key)">
            <span :class="['by-arrow-wrap', item._key === activeKey ? 'up' : 'down']"></span>
          </span>
          <div v-if="item._key === activeKey" ref="cascadeTreePopup" class="cascade--tree__popup" :style="{width: popupWidth, 'max-height': popupMaxHeight}">
            ...
            <div class="popup-close">
              <by-button class="by-button by-button--text" @click="showSecondSelection(activeKey)">关闭</by-button>
            </div>
          </div>
        </template>
      </div>
...
import ByCheckbox from '../../by-checkbox'
import ByButton from '../../by-button'
export default {
  name: 'ByCascadeTree',
  components: {
    'by-checkbox': ByCheckbox,
    'by-button': ByButton
  },
  props: {
    // v-model绑定的值，必传
